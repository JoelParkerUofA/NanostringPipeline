...
We demonstrate the order and structure of these steps in the diagram below. Please 
note, in this diagram the arrows do not represent a dependency but rather to show the
order the data is processed. For example, we do not need to run segment QC in order
to run probe QC. However, if both segment QC and probe QC are turned "ON", then the 
segment QC will be ran first. 


![Elephant](../Images/Proprocessing workflow.png)
...
The exploratory analysis allows us to visualize our data to give 
